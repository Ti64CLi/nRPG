...
void gDrawSprite(buffer_t *screen_buffer, int16_t x, int16_t y, sprite_t sprite, uint8_t zoom)
{
        ...
        for (uint16_t j = 0; j < sprite.width; j++)
        {
            for (uint8_t iz = 0; iz < zoom; iz++)
            {
                for (uint8_t jz = 0; jz < zoom; jz++)
                {
                    ...
                    if (xcoord > 0 && xcoord < screen_buffer->width && ycoord > 0 && ycoord < screen_buffer->height)
...
void gDrawTileMap(buffer_t *screen_buffer, tilemap_t tilemap, int16_t x, int16_t y, uint16_t tilemap_x, uint16_t tilemap_y, uint16_t tilemap_w, uint16_t tilemap_h, uint8_t zoom, int8_t xoffset, int8_t yoffset)
{
